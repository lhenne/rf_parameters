    ...
    def __init__(self) -> None:
        ...
        if any(self.method_calls):
            ...
            if self.method_calls[0]:
                self.data = self.data.assign(v1_start = np.nan, v1_end = np.nan, v1_duration = np.nan)
    ...
    def get_vowel_duration(self):
        ...
        * Return a DataFrame containing speaker, utterance, TextGrid-filename and vowel duration
        ...
        if isinstance(self.data, pd.DataFrame) and "filepath" in self.data.columns:
            ...
            for i, row in tqdm(self.data.iterrows(), desc="Extracting vowel intervals", total = len(self.data), leave = True, position = 0):
                ...
                for tiernum in range(1, numtiers+1):
                    ...
                    if tiername == "Vowel":
                        ...
                        if numintervals == 3:
                            v1_start = praat.call(textgrid, "Get start time of interval", tiernum, 2)
                            v1_end = praat.call(textgrid, "Get end time of interval", tiernum, 2)
